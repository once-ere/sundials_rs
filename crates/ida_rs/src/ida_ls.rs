/* -----------------------------------------------------------------
 * Translated from src/ida/ida_ls.c (IDA 7.7.0).
 * IDA's linear solver interface (IDALS): attach a SUNLinearSolver,
 * Jacobian and preconditioner plumbing, difference-quotient Jacobian
 * (dense + band, with IDA's two-vector (yy,yp) cj-weighted
 * perturbations) and Jacobian-times-vector approximations, and the
 * generic linit/lsetup/lsolve/lperf routines dispatched from IDASolve.
 * Translation conventions follow the donor cvode_ls.rs and the landed
 * kinsol_ls.rs; the interface hooks carry the IDALsMem explicitly per
 * the workspace take() convention:
 *
 ...
 * pointers the C code parks inside IDALsMem, see ida_ls_impl.rs.)
 * -----------------------------------------------------------------*/
...
pub fn IDASetLinearSolver(ida_mem: &mut IDAMem, LS: LinearSolver, A: Option<SUNMatrix>) -> i32 {
    ...
    /* In C the four system linear solver function fields (ida_linit /
       ...
       `iterative` flag stored below (ida_impl.rs contract). */
...
/* IDASetLSNormFactor sets or computes the factor to use when converting from
...
/*---------------------------------------------------------------
  ...
  they are passed as arguments here, see ida_ls_impl.rs.
...
/*---------------------------------------------------------------
  ...
  aliasing fix has no counterpart here.)
  ---------------------------------------------------------------*/
pub fn idaLsPSetup(
    ida_mem: &mut IDAMem,
    idals_mem: &mut IDALsMem,
...
/* idaLsInit — the interface hook dispatched by the main solver
   (ida.rs LsModule dispatch name for idaLsInitialize; C ida_linit). */
...
pub fn idaLsSetup(
    ...
    /* Call LS setup routine -- the LS will call idaLsPSetup if applicable:
       ...
       user pset is non-NULL) and maps a failure to SUNLS_PSET_FAIL_UNREC /
...
/*---------------------------------------------------------------
 ...
 `iterative`, and ida.rs guards the dispatch on
 idals_mem.iterative (ida_impl.rs contract).
---------------------------------------------------------------*/
...
mod tests {
    ...
    /* IDASetLinearSolver sets the C defaults (ida_ls.c lines 170-286):
    ...
    /* idaLsSetup drives the dense DQ Jacobian: for the linear test DAE
       ...
       nje/nreDQ/nstlj/tnlj updates follow ida_ls.c lines 1355-1425. */
    #[test]
    ...
    /* the internal DQ Jv product matches J*v for the linear test DAE
       (idaLsDQJtimes, ida_ls.c lines 1207-1255) */
    #[test]
