/* -----------------------------------------------------------------
 * Translated from src/ida/ida_nls.c (IDA 7.7.0), together with
 * ida.c's IDANls driver and the Newton solve loop of
 * sunnonlinsol_newton.c that the C code reaches through the
 * SUNNonlinearSolver ops table.  The IDA-specific callbacks
 * (idaNlsResidual, idaNlsLSetup, idaNlsLSolve, idaNlsConvTest) are
 * collapsed donor-style (cvode_nls.rs) into direct functions over
 * &mut IDAMem; control flow and arithmetic order match the C
 * sources statement for statement.
 *
 * Exported entry points for ida.rs:
 *   idaNlsInit(ida_mem: &mut IDAMem) -> i32      (C idaNlsInit)
 *   IDANls(ida_mem: &mut IDAMem) -> i32          (C ida.c IDANls,
...
const ZERO: f64 = 0.0; /* real 0.0 (ida.c) */
const ONE: f64 = 1.0; /* real 1.0    */
const TWENTY: f64 = 20.0; /* real 20.0   */
const HUNDRED: f64 = 100.0; /* real 100.0 (ida.c) */
...
/* idaNlsLSetup (ida_nls.c): wrapper around the lsetup dispatch.
...
/* idaNlsLSolve (ida_nls.c): wrapper around the lsolve dispatch.
...
/* idaNlsResidual (ida_nls.c): updates yy/yp from the current
...
/* idaNlsConvTest (ida_nls.c).  m is the current nonlinear iteration
...
pub fn IDANls(ida_mem: &mut IDAMem) -> i32 {
    ...
    /* initial guess for the correction to the predictor
       (ee is detached from IDAMem for the duration of the solve: it is
       the ycor vector the C code hands to SUNNonlinSolSolve) */
 ...
 * the IDA callbacks.  ycor = ida_ee (detached by IDANls),
...
mod tests {
    use super::*;
    use crate::ida_ls::{idaLs_AccessLMem, IDASetLinearSolver};
    ...
    /* IDASetNonlinearSolver stores the solver, clears ownership, sets
       maxiters = MAXIT = 4 and wires nls_res (ida_nls.c lines 44-132);
    ...
    /* IDANls performs the full Newton solve on a linear DAE through
       ...
       carry the corrected states (ida.c lines 2730-2807). */
    #[test]
    fn idanls_newton_linear_dae() {
        ...
        assert_eq!(IDASetLinearSolver(&mut ida_mem, ls, Some(a)), 0);
        {
            ...
            if let LsModule::Ls(m) = &mut lmem {
                assert_eq!(crate::ida_ls::idaLsInit(&mut ida_mem, m), 0);
            }
            ida_mem.ida_lmem = lmem;
        }
        let nls = SUNNonlinSol_Newton(&ida_mem.ida_yy, &sunctx);
