/* ------------------------------------------------------------------
 * Translation of examples/arkode/C_serial/ark_onewaycouple_mri.c
 * (SUNDIALS 7.7.0).
 *
 * Example problem:
 *
...
fn fs(_t: f64, y: &NVector, ydot: &mut NVector, _user_data: &mut UserData) -> i32 {
    let c1: f64 = 50.0; /* problem constant */
...
fn ans(t: f64, ytrue: &mut NVector) -> i32 {
    let c1: f64 = 50.0;
    let c2: f64 = 5051.0 / 2501.0;
    let c3: f64 = 49.0 / 2501.0;
    let c4: f64 = 51.0 / 2501.0;

    /* fill in the solution vector */
    ytrue.data[0] = (c1 * t).cos();
    ytrue.data[1] = (c1 * t).sin();
    ytrue.data[2] = c2 * (-t).exp() - c3 * (c1 * t).cos() + c4 * (c1 * t).sin();
...
fn err(y: &NVector, ytrue: &mut NVector, e: &mut f64) -> i32 {
    /* compute the error and store it in ytrue: the C
       N_VLinearSum(1, y, -1, ytrue, ytrue) aliases its output with
       ytrue -> in-place form (bitwise equal to C's VDiff kernel) */
    ytrue.linear_sum_with(-1.0, 1.0, y);
...
fn main() {
    ...
    let hf: f64 = 0.0001; /* fast step size */
    ...
    let mut ufid = std::fs::File::create("ark_onewaycouple_mri_solution.txt").expect("fopen");
    let _ = writeln!(ufid, "# t u v w maxerr");
    ...
    for _iout in 0..nt {
        ...
        if check_retval(retval, "ans") {
            break;
        }

        /* compute the error compared to the analytic solution */
        let retval = err(&y, &mut ytrue, &mut error);
        if check_retval(retval, "err") {
            break;
        }
