/* -----------------------------------------------------------------------------
 * Translation of examples/arkode/C_serial/ark_dissipated_exp_entropy.c
 * (SUNDIALS 7.7.0).
 * -----------------------------------------------------------------------------
...
fn main() {
    ...
    if implicit != 0 {
        ...
        let a_mat = SUNDenseMatrix(1, 1, &ctx);
    ...
    let mut ufid = std::fs::File::create("ark_dissipated_exp_entropy.txt").expect("fopen");
    let _ = writeln!(ufid, "# vars: t u entropy u_err delta_entropy");
