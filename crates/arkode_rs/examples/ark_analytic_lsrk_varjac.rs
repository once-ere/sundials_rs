/* -----------------------------------------------------------------
 * Translation of examples/arkode/C_serial/ark_analytic_lsrk_varjac.c
 * (SUNDIALS 7.7.0).
 *
...
use arkode_rs::arkode_lsrkstep_io::{
    ...
    LSRKStepSetMaxNumStages, LSRKStepSetSTSMethodByName,
};
...
fn dom_eig(
    ...
    let lambda = rdata[0]; /* set shortcut for stiffness parameter 1 */
    let alpha = rdata[1]; /* set shortcut for stiffness parameter 2 */
    *lambdaR = lambda - alpha * ((10.0 - t) / 10.0 * (-1.0f64).acos()).cos();
    *lambdaI = 0.0;
    0 /* return with success */
}
