/* -----------------------------------------------------------------
 * Translation of examples/arkode/C_serial/ark_KrylovDemo_prec.c
 * (SUNDIALS 7.7.0).
 *
 ...
 *
 * The ODE system is solved by ARKODE using Newton iteration and
 ...
 * options are tested. In the series of runs, ARKStepCreate,
 * SUNLinSol_SPGMR and ARKodeSetLinearSolver are called only for the
 * first run, whereas ARKStepReInit, SUNLinSol_SPGMRSetPrecType, and
 ...
 *
 * Translation notes (same as the cvKrylovDemo_prec port):
 * - The C demo stores the arkode_mem pointer inside the user data so
 *   that Precond can fetch the current error weights with
 *   ARKodeGetErrWeights. Rust callbacks cannot reach the integrator
 *   memory, so an equivalent user efun (ARKodeWFtolerances)
 *   reproduces the ARKodeSStolerances weights bit-for-bit
 *   (ewt_i = 1/(reltol*|y_i| + abstol), the exact arkEwtSetSS
 *   arithmetic) and keeps a snapshot in wdata.rewt, which is exactly
 *   what ARKodeGetErrWeights would return at Precond time.
...
use arkode_rs::arkode::{ARKodeEvolve, ARKodeFree, ARKodeWFtolerances};
use arkode_rs::arkode_arkstep::{ARKStepCreate, ARKStepReInit};
use arkode_rs::arkode_io::{
    ARKodeGetLastStep, ARKodeGetNumErrTestFails, ARKodeGetNumLinSolvSetups,
    ARKodeGetNumNonlinSolvConvFails, ARKodeGetNumNonlinSolvIters, ARKodeGetNumRhsEvals,
    ARKodeGetNumSteps, ARKodeGetWorkSpace, ARKodeSetMaxNumSteps, ARKodeSetNonlinConvCoef,
    ARKodeSetUserData,
};
use arkode_rs::arkode_ls::{
    ARKodeGetLinWorkSpace, ARKodeGetNumLinConvFails,
    ARKodeGetNumLinIters, ARKodeGetNumLinRhsEvals, ARKodeGetNumPrecEvals, ARKodeGetNumPrecSolves,
    ARKodeSetEpsLin, ARKodeSetLSNormFactor, ARKodeSetLinearSolver, ARKodeSetPreconditioner,
};
...
fn main() {
    ...
    /* Retrieve the command-line options (C: atoi(argv[1])) */
    ...
    /* In C arkode_mem is created on the first pass through the loop. */
    let mut arkode_mem: Option<Box<ARKodeMem>> = None;
    ...
    for jpre in SUN_PREC_LEFT..=SUN_PREC_RIGHT {
        for gstype in SUN_MODIFIED_GS..=SUN_CLASSICAL_GS {
            ...
            if firstrun {
                let mut am = ARKStepCreate(None, Some(f), T0, &c, &sunctx)
                    .expect("ARKStepCreate");

                let mut flag =
                    ARKodeSetUserData(&mut am, Some(Box::new(wdata_opt.take().unwrap())));
                if check_flag(flag, "ARKodeSetUserData") {
                    std::process::exit(1);
                }

                /* C: ARKodeSStolerances(arkode_mem, reltol, abstol); the user
                   efun computes the identical weights and snapshots them for
                   Precond */
                flag = ARKodeWFtolerances(&mut am, ewt);
                if check_flag(flag, "ARKodeSStolerances") {
                    std::process::exit(1);
                }

                flag = ARKodeSetMaxNumSteps(&mut am, 1000);
                if check_flag(flag, "ARKodeSetMaxNumSteps") {
                    std::process::exit(1);
                }

                flag = ARKodeSetNonlinConvCoef(&mut am, 1.0e-3);
                ...
                flag = ARKodeSetLinearSolver(&mut am, LS, None);
                if check_flag(flag, "ARKodeSetLinearSolver") {
                    std::process::exit(1);
                }

                flag = SUNLinSol_SPGMRSetGSType(&mut am, gstype);
                if check_flag(flag, "SUNLinSol_SPGMRSetGSType") {
                    std::process::exit(1);
                }

                flag = ARKodeSetEpsLin(&mut am, DELTA);
                if check_flag(flag, "ARKodeSetEpsLin") {
                    std::process::exit(1);
                }

                flag = ARKodeSetPreconditioner(&mut am, Some(Precond), Some(PSolve));
                if check_flag(flag, "ARKodeSetPreconditioner") {
                    std::process::exit(1);
                }

                /* Set the linear solver tolerance conversion factor */
                let nrmfac = match nrmfactor {
                    1 => (NEQ as f64).sqrt(), /* square root of vector length */
                    2 => -ONE,                /* compute with dot product */
                    _ => ZERO,                /* use the default */
                };

                flag = ARKodeSetLSNormFactor(&mut am, nrmfac);
                if check_flag(flag, "ARKodeSetLSNormFactor") {
                    std::process::exit(1);
                }

                arkode_mem = Some(am);
            } else {
                let am = arkode_mem.as_mut().unwrap();
                let mut flag = ARKStepReInit(am, None, Some(f), T0, &c);
                if check_flag(flag, "ARKStepReInit") {
                    std::process::exit(1);
                }

                flag = SUNLinSol_SPGMRSetPrecType(am, jpre);
                check_flag(flag, "SUNLinSol_SPGMRSetPrecType");
                flag = SUNLinSol_SPGMRSetGSType(am, gstype);
                if check_flag(flag, "SUNLinSol_SPGMRSetGSType") {
                    std::process::exit(1);
                }
            }
            ...
            /* Loop over output points, call ARKodeEvolve, print sample
               solution values. */
            let mut t = ZERO;
            let mut tout = T1;
            for iout in 1..=NOUT {
                let flag = ARKodeEvolve(am, tout, &mut c, &mut t, ARK_NORMAL);
                PrintOutput(am, t);
...
fn PrintOutput(arkode_mem: &mut ARKodeMem, t: f64) {
    let (mut nst, mut nfe, mut nfi, mut nni) = (0i64, 0i64, 0i64, 0i64);
    let mut hu = ZERO;

    let mut flag = ARKodeGetNumSteps(arkode_mem, &mut nst);
    ...
    flag = ARKodeGetLastStep(arkode_mem, &mut hu);
    check_flag(flag, "ARKodeGetLastStep");
...
fn PrintFinalStats(arkode_mem: &mut ARKodeMem) {
    let (mut lenrw, mut leniw) = (0i64, 0i64);
    let (mut lenrwLS, mut leniwLS) = (0i64, 0i64);
    let (mut nst, mut nfe, mut nfi, mut nsetups, mut nni, mut ncfn, mut netf) =
        (0i64, 0i64, 0i64, 0i64, 0i64, 0i64, 0i64);
    let (mut nli, mut npe, mut nps, mut ncfl, mut nfeLS) = (0i64, 0i64, 0i64, 0i64, 0i64);

    let mut flag = ARKodeGetWorkSpace(arkode_mem, &mut lenrw, &mut leniw);
    check_flag(flag, "ARKodeGetWorkSpace");
    ...
    flag = ARKodeGetLinWorkSpace(arkode_mem, &mut lenrwLS, &mut leniwLS);
    check_flag(flag, "ARKodeGetLinWorkSpace");
    flag = ARKodeGetNumLinIters(arkode_mem, &mut nli);
    check_flag(flag, "ARKodeGetNumLinIters");
    flag = ARKodeGetNumPrecEvals(arkode_mem, &mut npe);
    check_flag(flag, "ARKodeGetNumPrecEvals");
    flag = ARKodeGetNumPrecSolves(arkode_mem, &mut nps);
    check_flag(flag, "ARKodeGetNumPrecSolves");
    flag = ARKodeGetNumLinConvFails(arkode_mem, &mut ncfl);
...
/* Set weights identically to the internal arkEwtSetSS for
   ARKodeSStolerances(RTOL, ATOL) and keep the snapshot the C Precond
   would obtain from ARKodeGetErrWeights. */
...
/* SUNLinSol_SPGMRSetPrecType on the SPGMR solver attached to arkode_mem */
fn SUNLinSol_SPGMRSetPrecType(arkode_mem: &mut ARKodeMem, pretype: i32) -> i32 {
    match arkode_mem.lmem.as_mut() {
        Some(arkls_mem) => arkls_mem.LS.set_prec_type(pretype),
        None => SUN_ERR_ARG_OUTOFRANGE,
    }
}

/* SUNLinSol_SPGMRSetGSType on the SPGMR solver attached to arkode_mem */
fn SUNLinSol_SPGMRSetGSType(arkode_mem: &mut ARKodeMem, gstype: i32) -> i32 {
    match arkode_mem.lmem.as_mut() {
        Some(arkls_mem) => arkls_mem.LS.set_gs_type(gstype),
        None => SUN_ERR_ARG_OUTOFRANGE,
    }
}
...
fn Precond(
    ...
    /* C: flag = ARKodeGetErrWeights(wdata->cvode_mem, rewt); here wdata.rewt
...
/* Check function return value (flag < 0 means failure) */
