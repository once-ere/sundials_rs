/* -----------------------------------------------------------------
 * Translation of examples/arkode/C_serial/ark_robertson.c
 * (SUNDIALS 7.7.0).
 *
...
use arkode_rs::arkode_io::{
    ARKodePrintAllStats, ARKodeSetInitStep, ARKodeSetMaxErrTestFails, ARKodeSetMaxNonlinIters,
    ARKodeSetMaxNumSteps, ARKodeSetNonlinConvCoef, ARKodeSetPredictorMethod,
};
...
fn main() {
    ...
    /* Set routines */
    let flag = ARKodeSetInitStep(&mut arkode_mem, h0);
    ...
    let mut fid = std::fs::File::create("ark_robertson_stats.csv").unwrap();
