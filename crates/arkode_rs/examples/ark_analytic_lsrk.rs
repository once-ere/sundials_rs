/* -----------------------------------------------------------------
 * Translation of examples/arkode/C_serial/ark_analytic_lsrk.c
 * (SUNDIALS 7.7.0).
 *
...
use arkode_rs::arkode_lsrkstep_impl::ARKODE_LSRK_RKL_2;
use arkode_rs::arkode_lsrkstep_io::{
    LSRKStepSetDomEigFn, LSRKStepSetDomEigFrequency, LSRKStepSetDomEigSafetyFactor,
    LSRKStepSetMaxNumStages, LSRKStepSetSTSMethod,
};
...
#[allow(clippy::too_many_arguments)] /* fixed ARKDomEigFn signature */
fn dom_eig(
    _t: f64,
    _y: &NVector,
    _fn_: &NVector,
...
fn main() {
    ...
    let lambda: f64 = -1000000.0; /* stiffness parameter */
    ...
    let flag = LSRKStepSetDomEigFn(&mut arkode_mem, Some(dom_eig));
    assert!(flag >= 0, "LSRKStepSetDomEigFn failed with flag = {}", flag);

    /* Specify after how many successful steps dom_eig is recomputed
    Note that nsteps = 0 refers to constant dominant eigenvalue */
    let flag = LSRKStepSetDomEigFrequency(&mut arkode_mem, 0);
    ...
    let flag = ARKodeSetMaxNumSteps(&mut arkode_mem, 1000);
    ...
    let flag = LSRKStepSetSTSMethod(&mut arkode_mem, ARKODE_LSRK_RKL_2);
    assert!(flag >= 0, "LSRKStepSetSTSMethod failed with flag = {}", flag);
