/* -----------------------------------------------------------------
 * Translated from examples/kinsol/serial/kinFoodWeb_kry.c (KINSOL 7.7.0)
 *
 * Example (serial):
 *
...
const MX: i64 = 8; /* MX = number of x mesh points */
const MY: i64 = 8; /* MY = number of y mesh points */
...
/* IJ_V is defined in order to translate from the underlying 3D structure of
...
fn main() {
    ...
    /* Allocate and initialize user data block (data->rates is allocated
       here rather than after cc/sc as in the C main; order is
       inconsequential) */
    ...
    let mut retval = KINInit(&mut kmem, func, &cc);
    if check_retval(retval, "KINInit") {
        std::process::exit(1);
    }

    retval = KINSetUserData(&mut kmem, Some(Box::new(data)));
    ...
    /* Set the maximum number of restarts (C calls
       SUNLinSol_SPGMRSetMaxRestarts after KINSetLinearSolver; the LS
       object moves into kmem there, so set it just before attaching —
       the value is only read at solve time) */
    let maxlrst = 2;

    if let LinearSolver::Spgmr(s) = &mut LS {
        retval = s.set_max_restarts(maxlrst);
    ...
    retval = KINSetPreconditioner(&mut kmem, Some(PrecSetupBD), Some(PrecSolveBD));
    if check_retval(retval, "KINSetPreconditioner") {
        std::process::exit(1);
    }
...
fn PrintHeader(globalstrategy: i32, maxl: i32, maxlrst: i32, fnormtol: f64, scsteptol: f64) {
