/* -----------------------------------------------------------------
 * Translated from examples/kinsol/serial/kinLaplace_picard_bnd.c
 * (KINSOL 7.7.0)
 *
...
fn main() {
    ...
    println!("Solution method: Anderson accelerated Picard iteration with band linear solver.");
    ...
    let mut retval = KINSetMAA(&mut kmem, 3);
    if check_retval(retval, "KINSetMAA") {
        std::process::exit(1);
    }

    retval = KINInit(&mut kmem, func, &y);
...
fn PrintFinalStats(kmem: &mut KINMem) {
    ...
    let (mut lenrwB, mut leniwB) = (0i64, 0i64);
