/* -----------------------------------------------------------------
 * Translation of examples/cvodes/serial/cvsFoodWeb_ASAi_kry.c
 * (SUNDIALS 7.7.0).
 *
 ...
 * Here g(t,c) = c(ISPEC). During the forward run, G is computed as
 * phi(t_final) where d(phi)/dt is the 2-D Simpson integral of g.
 *
 ...
 *   reaches the backward problem's own user data.
...
use cvodes_rs::cvodea_io::{
    CVodeGetAdjCVodeBmem, CVodeSetMaxNumStepsB, CVodeSetUserDataB,
};
...
struct WebData {
    ...
    fsave: Vec<f64>,  /* [NEQ] */
    fBsave: Vec<f64>, /* [NEQ] */
    rewt: NVector,    /* NEQ+1 (forward weights snapshot) */
    vtemp: NVector,   /* NEQ+1 */
    rewtB: NVector,   /* NEQ (backward weights snapshot) */
    vtempB: NVector,  /* NEQ */
}
...
fn f(t: f64, c: &NVector, cdot: &mut NVector, user_data: &mut UserData) -> i32 {
    ...
    cdot.data[NEQ] = doubleIntgr(cdata, ISPEC, wdata);
...
fn Precond(
    ...
    let mut r0 = 1000.0 * SUNRabs(gamma) * uround * (NEQ + 1) as f64 * fac;
    if r0 == ZERO {
        r0 = ONE;
    }
...
fn PSolve(
    ...
    let intgr = doubleIntgr(&z.data, ISPEC, wdata);
    z.data[NEQ] = r.data[NEQ] + gamma * intgr;
...
fn PSolveB(
    ...
    let mut x = std::mem::take(&mut wdata.vtempB);
    GSIter(-gamma, z, &mut x, wdata);
    wdata.vtempB = x;
...
fn AllocUserData(sunctx: &SUNContext) -> WebData {
    ...
    WebData {
        ...
        fsave: vec![ZERO; NEQ],
        fBsave: vec![ZERO; NEQ],
        rewt: N_VNew_Serial((NEQ + 1) as i64, sunctx),
        vtemp: N_VNew_Serial((NEQ + 1) as i64, sunctx),
        rewtB: N_VNew_Serial(NEQ as i64, sunctx),
        vtempB: N_VNew_Serial(NEQ as i64, sunctx),
    }
}
...
fn PrintOutput(cB: &NVector, ns: usize, mxns: usize, wdata: &WebData) {
    ...
    for i in 1..=ns {
        ...
        println!("  lambda max = {}", fmt_e(cmax, 0, 6));
        println!("at");
...
fn main() {
    ...
    /* Initializations */
    let mut c = N_VNew_Serial((NEQ + 1) as i64, &sunctx);
    ...
    let retval = CVodeSetMaxNumStepsB(&mut cvode_mem, indexB, 1000);
    if check_retval(retval, "CVodeSetMaxNumStepsB") {
        return;
    }
