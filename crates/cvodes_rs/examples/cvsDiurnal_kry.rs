/* -----------------------------------------------------------------
 * Translated from examples/cvode/serial/cvsDiurnal_kry.c (CVODE 7.7.0)
 *
 * Example problem:
 *
...
/* Allocate memory for data structure of type UserDataStruct, and
   load problem constants (AllocUserData + InitUserData in the C). */
...
fn main() {
    ...
    retval = CVodeSetJacTimes(&mut cvode_mem, None, Some(jtv));
    if check_retval(retval, "CVodeSetJacTimes") {
        std::process::exit(1);
    }
