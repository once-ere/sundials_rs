/* -----------------------------------------------------------------
 * Translation of examples/cvodes/serial/cvsRoberts_FSA_dns_constraints.c
 * (SUNDIALS 7.7.0).
 *
 ...
 * Jacobian routine. The constraint y_i >= 0 is posed for all
 * components. It uses a scalar relative tolerance and a vector
 * absolute tolerance.
 *
 * Execution:
 *    % cvsRoberts_FSA_dns_constraints -nosensi
 *    % cvsRoberts_FSA_dns_constraints -sensi sensi_meth err_con
...
use cvodes_rs::cvodes::{
    CVode, CVodeCreate, CVodeFree, CVodeGetSens, CVodeInit, CVodeSensEEtolerances,
    CVodeSensInit1, CVodeWFtolerances,
};
use cvodes_rs::cvodes_io::{
    ...
    CVodeGetNumRhsEvalsSens, CVodeGetNumLinSolvSetups, CVodeGetNumStepSensSolveFails,
    CVodeGetNumStepSolveFails, CVodeGetNumSteps, CVodeGetSensNumErrTestFails,
    CVodeGetSensNumLinSolvSetups, CVodeGetSensNumNonlinSolvConvFails,
    CVodeGetSensNumNonlinSolvIters, CVodeGetSensNumRhsEvals, CVodeSetConstraints,
 ...
 */
fn print_final_stats(cvode_mem: &mut CVodeMem, sensi: bool) {
    let mut nst: i64 = 0;
    let mut nfe: i64 = 0;
    ...
    if sensi {
        ...
        CVodeGetSensNumNonlinSolvConvFails(cvode_mem, &mut nnfS);
        CVodeGetNumStepSensSolveFails(cvode_mem, &mut ncfnS);
    }
...
fn main() {
    ...
    print_final_stats(&mut cvode_mem, sensi);
