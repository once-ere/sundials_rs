/* -----------------------------------------------------------------
 * Translation of examples/cvodes/serial/cvsRoberts_ASAi_dns.c
 * (SUNDIALS 7.7.0).
 *
 ...
 * (adjoint) integration of
 *   d(lambda)/dt = - (df/dy)^T lambda - (dg/dy)^T
 *   lambda(tB0) = 0
 * and
 *   dG/dp = phi(t0) + int_t0^tB0 (df/dp)^T lambda dt
 * -----------------------------------------------------------------*/
...
use cvodes_rs::cvodes_io::{
    CVodePrintAllStats, CVodeSetMaxNumSteps, CVodeSetQuadErrCon, CVodeSetUserData,
};
...
const RTOL: f64 = 1e-6; /* scalar relative tolerance            */

const ATOL1: f64 = 1e-8; /* vector absolute tolerance components */
const ATOL2: f64 = 1e-14;
const ATOL3: f64 = 1e-6;
...
fn main() {
    ...
    /* Create dense SUNMatrix and dense SUNLinearSolver for use by CVode */
    ...
    let mut fid = std::fs::File::create("cvsRoberts_ASAi_dns_fwd_stats.csv").expect("fopen");
    ...
    if let Some(bmem) = CVodeGetAdjCVodeBmem(&mut cvode_mem, indexB) {
        let mut stdout = std::io::stdout();
        let _ = CVodePrintAllStats(bmem, &mut stdout, SUN_OUTPUTFORMAT_TABLE);
    }

    /* Print final statistics to a file in CSV format */
    if let Some(bmem) = CVodeGetAdjCVodeBmem(&mut cvode_mem, indexB) {
        let mut fid = std::fs::File::create("cvsRoberts_ASAi_dns_bkw1_stats.csv").expect("fopen");
        let _ = CVodePrintAllStats(bmem, &mut fid, SUN_OUTPUTFORMAT_CSV);
    }
    ...
    /* Print final statistics to a file in CSV format */
    if let Some(bmem) = CVodeGetAdjCVodeBmem(&mut cvode_mem, indexB) {
        let mut fid = std::fs::File::create("cvsRoberts_ASAi_dns_bkw2_stats.csv").expect("fopen");
        let _ = CVodePrintAllStats(bmem, &mut fid, SUN_OUTPUTFORMAT_CSV);
    }
