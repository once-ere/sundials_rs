/* -----------------------------------------------------------------
 * Translated from examples/cvode/serial/cvAdvDiff_bndL.c (CVODE 7.7.0)
 *
 * Example problem:
 *
 ...
 * iteration with the LAPACKBAND linear solver in the original C.
 * The pure-Rust build has no LAPACK; SUNLinSol_LapackBand maps onto
 * the native band LU (SUNLinSol_Band), which performs the same
 * factorization.
...
fn PrintFinalStats(cvode_mem: &mut CVodeMem) {
    ...
    println!("nni = {:<6} ncfn = {:<6} netf = {}", nni, ncfn, netf);
}
...
fn main() {
    ...
    /* Create SUNLinSol_LapackBand solver object for use by CVode
       (native band LU in the pure-Rust build) */
