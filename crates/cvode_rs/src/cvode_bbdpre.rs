/* -----------------------------------------------------------------
 * Translated from src/cvode/cvode_bbdpre.c (CVODE 7.7.0).
 * Band-block-diagonal preconditioner, i.e. a block-diagonal matrix
 * with banded blocks, for use with CVODE and the CVLS linear solver
 * interface. This pure-Rust build is the serial reduction: a single
 * block of dimension n_local.
 *
 ...
 * cvode_ls.rs dispatch to CVBBDPrecSetup / CVBBDPrecSolve.
 * -----------------------------------------------------------------*/
use crate::cvode_bbdpre_impl::{CVBBDPrecData, CVCommFn, CVLocalFn};
...
/* Error messages (cvode_bbdpre_impl.h) */
