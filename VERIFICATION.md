...
| ark_brusselator1D_imexmri | IDENTICAL x7 (shipped refs; all seven solve_type variants at hs=0.001 byte-exact on first run — MIS (exp slow/ESDIRK-3-3 fast), MRI-GARK-ESDIRK34a solve-decoupled over ERK-3-3 and ESDIRK-3-3 inners, IMEX-MRI3b and IMEX-MRI4 over ERK and DIRK/CASH(5,3,4) inners; custom user Butcher tables via ARKStepSetTables, MRIStepSetCoupling with loaded MRI/IMEX-MRI tables, band Newton solves on both time scales) |
...
| ark_damped_harmonic_symplectic | LOCAL-C (byte-identical to a fresh 7.7.0 C build; shipped ref stale SUN_TABLE_WIDTH 28 vs 29, all numeric values identical. SPRKStep order-4 on the TIME-DEPENDENT damped oscillator (t-dependent split RHS + Hamiltonian tracking) over t=[0,10pi]) |
...
| ark_harmonic_symplectic | LOCAL-C (byte-identical to a fresh 7.7.0 C build; shipped ref stale SUN_TABLE_WIDTH 28 vs 29, all numeric values identical. SPRKStep order-4 default on the harmonic oscillator, tstop mode, L2 error vs analytic solution with in-loop tolerance check) |
