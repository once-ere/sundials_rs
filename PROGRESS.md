...
- [x] arkode.c — PARTS I-II committed (vector utils, ARKodeGetDky, arkCreate, arkEwtSetSS/SV/SmallReal + arkRwtSet family with donor in-place idiom; efun=None means internal dispatch). PART III adds arkInit + arkCheckTimestepper/Nvector + arkAllocVectors/arkFreeVectors (rwt_is_ewt = rwt left unallocated). PART V adds the full Evolve chain: ARKodeEvolve, arkInitialSetup, arkStopTests, arkCompleteStep, arkHandleFailure, arkCheckConvergence/Constraints/TemporalError, ark_rfun_apply_yn (relaxation call site stubs to ARK_RELAX_MEM_NULL until arkode_relaxation.c lands). PART VI adds ARKodeSStolerances/SVtolerances/WFtolerances + ARKodeFree; arkode_io.rs PART II adds SetUserData/SetInitStep/SetFixedStep/SetMaxNumSteps/SetStopTime/SetInterpolantDegree. PART VII adds ARKodeReset (arkInit RESET_INIT + optional step_reset op). PART VIII adds ARKodeCreateMRIStepInnerStepper + the ark_MRIStepInner* op family (wraps an owned Box<ARKodeMem> as MRIStepInnerStepper content). PART IX adds ARKodeResize + arkResizeVectors (per-field detach around arkResizeVec; ycur re-created since the Rust ycur is owned where C aliases the user's resized vector; constraints freed as in C). PART X adds ARKodeResStolerance/ResVtolerance/ResFtolerance (rwt detach from ewt + arkRwtSet internal dispatch) and ARKodePrintMem (general/counter/time-stepping fields + arkPrintAdaptMem/arkPrintRootMem/arkInterpPrintMem + step_printmem). COMPLETE — full public-API audit vs include/arkode/*.h shows zero missing symbols (ARKodeSetAdjointCheckpointScheme excluded with the adjoint half)
- [x] arkode_io.c — PARTS I-III committed (SetDefaults, essential setters, arkReplaceAdaptController, ARKodePrintAllStats + sunfprintf helpers). PART IV adds ARKodeSetStepDirection/ARKodeGetStepDirection (controller reset + step_setstepdirection op dispatch). PART V adds ARKodeGetAccumulatedError/ARKodeResetAccumulatedError. PART VI adds the ~30 remaining Set routines needed by the CLI module and PART VII adds ARKodeSetConstraints + ARKodeGetNumConstrFails (SetOrder/SetMaxHnilWarns/SetInterpolateStopTime/SetMaxNumConstrFails/SetAdaptivityAdjustment/SetSmallNumEFails/SetMaxConvFails/SetMinStep/SetMaxStep/CFL/safety/error-bias/growth/reduction/fixed-step-bounds families, SetPredictorMethod with the ARK_INTERP_NONE check, ClearStopTime, SetNoInactiveRootWarn, SetAccumulatedErrorType). PART VIII adds the deprecated arkSetAdaptivityMethod (full PID/PI/I/ExpGus/ImpGus/ImExGus SUNAdaptController switch with workspace accounting; ARKStepSetAdaptivityMethod/ERKStepSetAdaptivityMethod wrappers in the stepper io modules). PART IX adds ARKodeSetAdaptController/SetAdaptControllerByName (11 named controllers), SetRootDirection, SetPreStepFn/SetPostStepFn/SetPreRhsFn, SetPostprocessStepFn/StageFn, SetStabilityFn, ComputeState, GetActualInitStep/GetCurrentState/GetCurrentTime/GetLastState/GetLastTime/GetTolScaleFactor/GetErrWeights/GetResWeights/GetStepStats/GetNumExpSteps/GetNumAccSteps/GetUserData/GetStageIndex, ARKodeGetReturnFlagName. WriteParameters + SetOptions landed earlier (see arkode_arkstep_io/arkode_cli rows). COMPLETE — audit-clean except the adjoint-half ARKodeSetAdjointCheckpointScheme
...
- [x] arkode_erkstep.c — PART I committed (ERKStepCreate/ReInit, erkStep Init/FullRHS/TakeStep/Resize/Free/PrintMem, SetButcherTable/CheckButcherTable/ComputeSolutions, ApplyForcing/SetInnerForcing; step_mem take/put-back with release around the FullRHS re-entry). Excluded (structural, see excluded (arkode)): TakeStep_Adjoint/fe_Adj/CreateAdjointStepper (ManyVector). RelaxDeltaE/GetOrder/SetRelaxFn landed with arkode_relaxation.c
- [x] arkode_erkstep_io.c — PART I committed (erkStep SetDefaults/SetOrder/GetNumRhsEvals/GetEstLocalErrors/GetStageIndex/PrintAllStats/WriteParameters + ERKStepSetTable/Num/Name, GetCurrentButcherTable (copy), GetTimestepperStats). SetOptions landed with arkode_cli.c, SetRelaxFn with arkode_relaxation.c; public-API audit shows no missing ERKStep* symbols
...
- [x] arkode_ls.c — PART I: system-matrix half (ARKodeSetLinearSolver + Set*/Get* option-stat families + GetLinReturnFlagName, arkLsDQJac dense/band, arkLsDQJtimes, arkLsLinSys, arkLsInitialize/Setup/Solve/Free; iterative solve via RefCell ATimes/PSolve closures with s1=rwt s2=ewt; psetup jcur write-back via Rust-only step_setjcur op). PART II adds the complete mass half: ARKodeSetMassLinearSolver/SetMassFn + mass Set/Get families, arkLsMTimes, arkLsMassInitialize/Setup/Solve (direct + iterative closures)/Free, mass hooks in the system half (A/M compatibility checks, M shortcut into linsys, MTimes in the ATimes closure). arkLsDQJtimes takes ewt as an argument (detached during iterative solves). PART III adds the matrix-embedded solve dispatch: arkLsSolve routes LinearSolver::Custom through the pinned CustomLinSol adaptation (solve receives (t, gamma, user_data) instead of C's stored arkode_mem back-pointer; gamma via the step_getgammas op)
- [x] arkode_arkstep_impl.h — committed (ARKodeARKStepMem incl. mass fn-pointer fields + MAXCOR/CRDOWN/DGMAX/RDIV/MSBP/NLSCOEF + MASS_* + ARKSTEP_DEFAULT_* table IDs; crate->crate_ rename; lmem hoisted per Addendum C.1; Xvecs/adj_fe/fn_implicit/mass-solver fields per modeling notes)
- [x] arkode_arkstep.c — PART I committed (ARKStepCreate/ReInit, arkStep Init/FullRHS/TakeStep_Z/ComputeState/Free/PrintMem, AttachLinsol/DisableLSetup/GetLmem/GetImplicitRHS/GetGammas + Rust-only SetJcur, SetButcherTables/CheckButcherTables, Predict (arkPredict_* ported into arkode.rs), StageSetup, ComputeSolutions; identity-mass paths — mass branches follow C's NULL paths; linit/minit run with step_mem re-installed so the ARKLS step_disablelsetup/step_disablemsetup re-entries take effect — found via ark_analytic_mels nsetups). Excluded (structural, see excluded (arkode)): TakeStep_ERK_Adjoint/fe_Adj/CreateAdjointStepper (adjoint), AttachMasssol/mass init-setup (ARKLS mass half). Reset: C installs no arkStep step_reset op — ARKodeReset drives ARKStep through the base arkInit(RESET_INIT) path, matching C (step_reset installers: mristep + forcingstep only, both ported). arkStep_Resize landed with ARKodeResize (sdata/zpred/zcor/Fe/Fi resize + owned-NLS Newton recreation); SetInnerForcing/ApplyForcing landed with arkode_sunstepper.c; RelaxDeltaE/GetOrder landed with arkode_relaxation.c
- [x] arkode_arkstep_io.c — PART I committed (arkStep_SetDefaults + full Set/Get op families, PrintAllStats, WriteParameters, ARKStepGetNumRhsEvals/GetCurrentButcherTables/GetTimestepperStats; arkode_io.rs gained the ARKodeSet/Get implicit-solver dispatch wrappers + ARKodeWriteParameters + ARKodeGetNumSteps/StepAttempts/ErrTestFails). PART II adds ARKStepSetExplicit/SetImplicit/SetImEx (with tolerance re-attach), ARKStepSetTables/SetTableNum/SetTableName; arkode_io.rs gains ARKodeSetInterpolantType + ARKodeGetNumStepSolveFails + ARKodeSetMaxErrTestFails + ARKodeGetNumGEvals/GetRootInfo. SetOptions landed with arkode_cli.c, SetRelaxFn with arkode_relaxation.c; public-API audit shows no missing ARKStep* symbols
- [x] arkode_arkstep_nls.c — PART I committed (SetNonlinearSolver/SetNlsRhsFn/SetNlsSysFn/GetNonlinearSystemData, NlsInit, Nls + inlined SUNNonlinSolSolve_Newton loop with ARKStep Sys/LSetup/LSolve/CTest callbacks; residual MassIdent + TrivialPredAutonomous variants; lsetup/lsolve wrappers re-install step_mem around ARKLS op re-entries). PART II adds the inlined SUNNonlinSolSolve_FixedPoint loop (Anderson acceleration via the core FixedPointSolver) + arkStep_NlsFPFunction MassIdent/TrivialPredAutonomous variants. Excluded (structural): MassFixed/MassTDep residual and FP variants (ARKLS mass half — sole consumer is the SuperLU-MT-only mass-matrix example)
...
- [x] arkode_mri_tables.c (+ .def) — committed (MRIStepCouplingMem with nested-Vec W/G tensors and MERK groups; Alloc/Create/MIStoMRI/Copy/Space/Free/Write + LoadTable/LoadTableByName with all 26 tables incl. MERK21-54 and IMEX-MRI-SR21/32/43; GetStageType/GetStageMap; `type` field renamed `type_`)
- [x] arkode_mristep_impl.h — committed (ARKodeMRIStepMem + MRIStepInnerStepper base class with fn-pointer ops; Fse/Fsi as owning Vecs — C's unify_Fs aliasing stores the unified array in Fse with every Fsi access routed through an `if unify_Fs` alias branch; lmem hoisted per Addendum C.1)
- [x] arkode_mristep.c — committed (MRIStepCreate/ReInit, mriStep Init/FullRHS/UpdateF0/Reset/ComputeState/Free/PrintMem/AttachLinsol/GetLmem/GetGammas(+Rust-only SetJcur)/ComputeH0/SlowRHS/Hin, all three TakeStep routines (MRIGARK with tempv4 mem::swap embedding trick, MRISR, MERK), SetCoupling/CheckCoupling, StageERKFast/ERKNoFast/DIRKNoFast (DIRKFast errors as in C), ComputeInnerForcing/RKCoeffs/Predict/StageSetup, the complete MRIStepInnerStepper function family (Create/CreateFromSUNStepper wraps an owned SUNStepper/Free/Set*Fn/AddForcing/Evolve/FullRhs/Reset/error accessors/AllocVecs), ApplyForcing coeffs helper, SetInnerForcing. Deferred: mriStep_Resize (with ARKodeResize))
- [x] arkode_mristep_io.c — committed (MRIStepSetCoupling/SetPreInnerFn/SetPostInnerFn, GetCurrentCoupling (returns a copy)/GetLastInnerStepFlag/GetNumInnerStepperFails, full mriStep_Set*/Get* op family, PrintAllStats/WriteParameters, SetOptions (coupling_table_name key); deprecated MRIStep* aliases omitted; SetAdaptController passes non-MRI controllers to arkReplaceAdaptController and reports MRI-H-TOL as unsupported pending the controller wrapper)
- [x] arkode_mristep_nls.c — committed (SetNonlinearSolver/SetNlsRhsFn/GetNonlinearSystemData, NlsInit, mriStep_Nls with inlined Newton and Anderson fixed-point loops per the arkstep_nls donor; LSetup/LSolve wrappers re-install step_mem around ARKLS re-entries; Residual/FPFunction/ConvTest with Fsi accesses routed through the unify_Fs alias)
- [x] arkode_mristep_controller.c — committed (pinned adaptation: C's SUNAdaptController_MRIStep wrapper stores raw ark_mem/step_mem back-pointers; the Rust wrapper collapses onto storing the MRI-H-TOL controller itself in hadapt_mem.hcontroller (owned) with the estimatestep/updateh ops dispatched from arkAdapt/arkCompleteStep by controller type — SUNAdaptController_EstimateStep_MRIStep/UpdateH_MRIStep read/write inner_rtol_factor/inner_dsm/inner_rtol_factor_new through the taken step_mem; gettype/reset/write/space act on the stored controller via generic dispatch. mriStep_SetAdaptController now accepts MRI-H-TOL controllers)
...
- [x] arkode_relaxation.c — committed (full module: ARKodeSetRelaxFn + Set/Get option-stat families, arkRelaxCreate (relax_mem allocated in place as Option<ARKodeRelaxMem>; +12/+14 lrw/liw), arkRelax driver (detach/put-back around the solve; TRY_AGAIN retry with eta_fail; h/dsm/ycur relax updates with ycur.linear_sum_with for C's aliased N_VLinearSum), arkRelaxSolve with Newton and Brent solvers, residual/Jacobian over tempv2-4, arkRelaxPrintAllStats. Stepper halves: arkStep_RelaxDeltaE (stored z[i] stages for implicit/IMEX/MASS_FIXED, explicit stage reconstruction via N_VLinearCombination, msolve hook) + arkStep_GetOrder + arkStep_SetRelaxFn; erkStep_RelaxDeltaE + erkStep_GetOrder + erkStep_SetRelaxFn; both steppers now install step_setrelaxfn and arkstep flips step_supports_relaxation to SUNTRUE. Driver call sites live: arkRelax in the ARKodeEvolve attempt loop, relax stats in ARKodePrintAllStats. Brent's C double-increment of num_relax_fn_evals kept bug-faithful. All three entropy examples byte-identical to a fresh local 7.7.0 C build (shipped refs stale))
- [x] arkode_user_controller.c/.h — committed (ARKUserControlContent + all six controller ops; same pinned adaptation as the MRIStep controller: the C wrapper's ark_mem back-pointer becomes call-site dispatch — the content box lives in the Rust-only hadapt_mem.usercontrol slot, arkAdapt/arkCompleteStep dispatch estimatestep/updateh with ark_mem in scope, and the reset/write/space forwarders hook the existing hcontroller call sites. arkSetAdaptivityFn (deprecated ARKAdaptFn path, I-controller fallback on NULL hfun) ported into arkode_io.rs with ARKStepSetAdaptivityFn/ERKStepSetAdaptivityFn wrappers; controller-removal accounting extended in arkReplaceAdaptController/arkSetAdaptivityMethod. No serial example exercises the deprecated path; regression-verified against the full arkode harness)
...
halves stay unported (incl. ARKodeSetAdjointCheckpointScheme, whose only callers are those halves). CVODES adjoint (cvodea) covers ASA end-to-end
