...
- [ ] arkode_arkstep_io.c — PART I committed (arkStep_SetDefaults + full Set/Get op families, PrintAllStats, WriteParameters, ARKStepGetNumRhsEvals/GetCurrentButcherTables/GetTimestepperStats; arkode_io.rs gained the ARKodeSet/Get implicit-solver dispatch wrappers + ARKodeWriteParameters + ARKodeGetNumSteps/StepAttempts/ErrTestFails). Remaining: ARKStepSetTables/SetTableNum/SetTableName, SetOptions (CLI), SetRelaxFn (relaxation), deprecated ARKStep* aliases
- [ ] arkode_arkstep_nls.c — PART I committed (SetNonlinearSolver/SetNlsRhsFn/SetNlsSysFn/GetNonlinearSystemData, NlsInit, Nls + inlined SUNNonlinSolSolve_Newton loop with ARKStep Sys/LSetup/LSolve/CTest callbacks; residual MassIdent + TrivialPredAutonomous variants; lsetup/lsolve wrappers re-install step_mem around ARKLS op re-entries). Deferred: fixed-point sysfn family + FP solve loop (ark_brusselator_fp), MassFixed/MassTDep residuals
- [ ] arkode_sprk.c — todo
